...
// Keep helper logic in sync with the boutique page
const brandKey = (b) =>
  (b || 'unknown')
    .trim()
    .toLowerCase()
...
export default function BrandPage({ params }) {
  ...
  const urlStrictKey = decodeURIComponent(params.brandKey || '');
  ...
  useEffect(() => {
    (async () => {
      ...
      // 1) Find the boutique owner (by username)
      ...
      // 2) Load all fragrances for that owner (we'll filter in JS by normalized brand)
  ...
  // Normalize and filter by both strict + canonical keys
  ...
  // Choose a nice display name from any match
  const displayBrand = useMemo(() => {
    ...
    // Use the most common display brand among matches
