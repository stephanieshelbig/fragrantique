...
export async function GET(req) {
  try {
    ...
    let prof = null;
    {
      const { data, error } = await supabase.from('profiles').select('id, username, email').eq('username', username).maybeSingle();
      if (error) return json(500, { ok: false, error: `profiles(username) ${error.message}` });
      if (data?.id) prof = data;
    }
    if (!prof) {
      const { data, error } = await supabase.from('profiles').select('id, username, email').eq('email', email).maybeSingle();
    ...
    if (prof?.id) {
      const { data: links, error: lErr } =
        await supabase.from('user_fragrances').select('fragrance_id').eq('user_id', prof.id);
    ...
    if (!reps.length) {
      mode = 'global';
      const { data: fragsAll, error: gErr } =
        await supabase.from('fragrances')
          .select('id, brand, name, image_url, image_url_transparent, updated_at, created_at');
