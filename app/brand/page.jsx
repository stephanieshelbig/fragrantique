...
// Fetch every row from Supabase in batches.
// This avoids the PostgREST/Supabase row limit hiding newer fragrances.
...
export default function BrandClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // -------------------------------------------------------
        // 2. Load ALL of Stephanie's fragrance links
        //
        // IMPORTANT:
        // The old version used one Supabase request here.
        // Large collections can exceed Supabase/PostgREST's
        // response limit, causing newer fragrances to disappear.
        //
        // We now fetch in batches of 1,000.
        // -------------------------------------------------------
        ...
        // -------------------------------------------------------
        // 3. Convert the join rows into fragrance records
        // -------------------------------------------------------
        const fragrances = (rows || [])
          .map((row) => row.fragrance)
          .filter(Boolean);

        setLinks(fragrances);
      } catch (error) {
        console.error("Brand index load error:", error);

        if (!cancelled) {
          setLoadError(
            error?.message ||
              "There was a problem loading the brand index."
          );
  ...
  const brands = useMemo(() => {
    ...
    return Array.from(map.entries()).sort((a, b) =>
      a[1].display
        .toLowerCase()
        .localeCompare(b[1].display.toLowerCase())
    );
  }, [links]);
    ...
    <div className="min-h-screen bg-[#fdfcf9]">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <p className="opacity-70 text-base text-[#182A39]">
          ...
          ’s boutique. BEST VIEWED IN LANDSCAPE MODE.
        </p>

        {loadError && (
          <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">
            Brand index load error: {loadError}
          </div>
        )}
