...
/** Build the on-brand slug exactly like the SQL function does */
...
export default function FragranceDetail({ params }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // ------------------------------------------
      // Load fragrance by slug (preferred) or UUID (back-compat)
      // ------------------------------------------
      let f = null;
      ...
      const { data: bySlug, error: slugErr } = await supabase
        ...
        .ilike('slug', slugParam) // case-insensitive to allow Electimuss-Celestial vs electimuss-celestial
        .maybeSingle();

      if (!slugErr && bySlug) {
        f = bySlug;
      } else {
        // If it looks like a UUID, fetch by id (old links), then redirect to slug
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(slugParam);
        if (isUuid) {
          ...
          if (byId) {
            f = byId;
            const desired = byId.slug && byId.slug.trim().length
              ? byId.slug
              : makeSlug(byId.brand, byId.name);
            if (desired && desired !== slugParam) {
              // Replace URL with pretty slug
      ...
      // Decant/options for this fragrance
      if (f?.id) {
  ...
  function handleAddToCart() {
    ...
    if (opt.quantity !== null && typeof opt.quantity === 'number') {
      ...
      if (remaining <= 0) { setMsg(`"${opt.label}" is already at the limit in your cart (${alreadyInCart}/${opt.quantity}).`); return; }
      if (q > remaining) { setMsg(`Only ${remaining} left for "${opt.label}" (you already have ${alreadyInCart} in your cart).`); return; }
    }
  ...
  async function saveOption(row) {
    ...
    const up = {
      ...
      price_cents: typeof row.price_cents === 'number' ? row.price_cents : dollarsToCents(row.price_dollars || ''),
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <div className="flex gap-6">
        <div className="relative w-44 sm:w-52 md:w-56 aspect-[3/5]">
          ...
          <img
            src={frag.image_url_transparent || frag.image_url || '/bottle-placeholder.png'}
            alt={frag.name}
          ...
          <div className="p-3 rounded border bg-white space-y-3">
            ...
            {isOwner && (
              <div className="space-y-4">
                ...
                <div className="text-xs opacity-60">Visitors won’t see prices here. They’ll just pick an option and quantity, then add to cart.</div>
              </div>
            )}
          </div>
        </div>
