...
export function renderOrderHtml({ sessionId, buyerEmail, amountTotal, currency, items, shipping }) {
  ...
  <div style="font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif">
    ...
    <div style="margin-top:18px">
      <a href="${SITE}/admin/orders" style="color:#111">Open Admin Orders →</a>
    </div>
    <div style="margin-top:6px">
      <a href="${SITE}/u/stephanie" style="color:#111">Open Boutique →</a>
    </div>
  </div>
  `;
}
...
export async function sendOrderEmail({ to, subject, html }) {
  ...
  const from = process.env.FROM_EMAIL || `Fragrantique Orders <${user || 'no-reply@fragrantique.net'}>`;
